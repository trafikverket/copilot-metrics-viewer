  ...
  try {
    ...
    const statusCode =
      error && typeof error === "object" && "statusCode" in error
        ...
        : 500;
    return new Response("Error fetching teams data: " + errorMessage, {
      status: statusCode,
    });
  }
});
...
export async function getTeams(
  ...
  if (!options.scope && config.public.scope)
    options.scope = config.public.scope as Scope;
  if (!options.githubOrg && config.public.githubOrg)
    options.githubOrg = config.public.githubOrg;
  if (!options.githubEnt && config.public.githubEnt)
    options.githubEnt = config.public.githubEnt;
    ...
    const teams: Team[] = [
      {
        name: "The A Team",
        slug: "the-a-team",
        description: "A team of elite agents",
      },
      {
