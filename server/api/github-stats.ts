  ...
  const totals = metrics.reduce(
    (acc, metric) => {
      ...
      if (metric.copilot_dotcom_pull_requests?.repositories) {
        acc.totalPRSummariesCreated +=
          metric.copilot_dotcom_pull_requests.repositories.reduce(
            (repoSum, repo) => {
              return (
                repoSum +
                (repo.models?.reduce((modelSum, model) => {
  ...
  for (const metric of metrics) {
    ...
    metric.copilot_ide_code_completions?.editors?.forEach((editor) => {
      editor.models?.forEach((model) => {
        ...
        modelMaps.ideCodeCompletion.get(key).total_engaged_users +=
  ...
  const agentModeChartData = {
    ...
    datasets: [
      ...
      },
      {
        label: "IDE Chat",
        data: metrics.map(
          (metric) => metric.copilot_ide_chat?.total_engaged_users || 0,
        ),
      ...
      },
      {
        ...
        data: metrics.map(
          (metric) => metric.copilot_dotcom_chat?.total_engaged_users || 0,
        ),
