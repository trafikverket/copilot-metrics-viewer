...
if (typeof globalThis.$fetch === "function") {
  ...
  const wrapped: typeof globalThis.$fetch = ((url: unknown, opts: unknown) => {
    ...
    if (isGitHub(str)) {
      return Promise.reject(
        new Error(
          `Blocked external GitHub API call during tests via $fetch: ${str}`,
        ),
      );
    }
  ...
  (wrapped as unknown as { raw?: unknown }).raw = (
    original$fetch as unknown as { raw?: () => unknown }
  ).raw?.bind(original$fetch);
  (wrapped as unknown as { create?: unknown }).create = (
    original$fetch as unknown as { create?: () => unknown }
  ).create?.bind(original$fetch);
