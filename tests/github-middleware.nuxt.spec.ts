...
describe("GitHub middleware authentication guard", () => {
  it("skips authentication for health endpoints", async () => {
    ...
    (authenticateAndGetGitHubHeaders as any).mockImplementation(() => {
      throw new Error("authenticate called unexpectedly");
    });
  ...
  it("requires authentication for other api routes", async () => {
    ...
    (authenticateAndGetGitHubHeaders as any).mockResolvedValue(
      new Headers({ Authorization: "token x" }),
    );
