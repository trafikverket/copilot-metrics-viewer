...
describe("Metrics Auth-Bound Cache Key", () => {
  ...
  it("filters out undefined/empty query params", () => {
    const key = buildMetricsCacheKey(
      path,
      { since: "2024-01-01", empty: "", undef: undefined },
      "token x",
    );
  ...
  it("merges existing path query params with provided query object without duplication", () => {
    ...
    const key = buildMetricsCacheKey(
      pathWithQuery,
      { githubOrg: "test-org", since: "2024-01-01" },
      "token z",
    );
  ...
  it("sorts final query params for stable ordering", () => {
    const key1 = buildMetricsCacheKey(
      "/api/metrics?b=2&a=1",
      { c: "3" },
      "token sort",
    );
    const key2 = buildMetricsCacheKey(
      "/api/metrics?a=1&b=2",
      { c: "3" },
      "token sort",
    );
