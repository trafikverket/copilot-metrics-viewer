...
export class Options {
  ...
  static fromRoute(
    ...
    if (route.params.org) {
      ...
      if (config.public.githubTeam)
        options.githubTeam = config.public.githubTeam;
    }
  ...
  toParams(): Record<string, string> {
    ...
    if (this.excludeHolidays)
      params.excludeHolidays = String(this.excludeHolidays);
  ...
  toObject(): OptionsData {
    ...
    if (this.isDataMocked !== undefined)
      result.isDataMocked = this.isDataMocked;
    ...
    if (this.excludeHolidays !== undefined)
      result.excludeHolidays = this.excludeHolidays;
  ...
  getApiUrl(): string {
    ...
    switch (this.scope) {
      ...
      case "team-enterprise":
        if (!this.githubEnt || !this.githubTeam) {
          throw new Error(
            "GitHub enterprise and team must be set for team-enterprise scope",
          );
        }
    ...
    if (this.since || this.until) {
      const sinceParam = this.since
        ? `since=${encodeURIComponent(this.since)}`
        : "";
      const untilParam = this.until
  ...
  getTeamMembersApiUrl(): string {
    ...
    switch (this.scope) {
      ...
      case "organization":
        if (!this.githubOrg || !this.githubTeam) {
          throw new Error(
            "GitHub organization and team must be set for organization scope",
          );
        }
      ...
      case "enterprise":
        if (!this.githubEnt || !this.githubTeam) {
          throw new Error(
            "GitHub enterprise and team must be set for enterprise scope",
          );
        }
  ...
  validate(): { isValid: boolean; errors: string[] } {
    ...
    if (
      this.scope === "team-organization" ||
      this.scope === "team-enterprise"
    ) {
      if (!this.githubTeam) {
