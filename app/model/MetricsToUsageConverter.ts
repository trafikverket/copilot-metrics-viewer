...
import { BreakdownData, Metrics } from "@/model/Metrics";
  ...
  try {
    ...
    const usageData: Metrics[] = copilotMetrics.map((metric) => {
      ...
      const totalChatInsertions =
        metric.copilot_ide_chat?.editors?.reduce((sum, editor) => {
          if (editor && editor.models) {
            return (
              sum +
              editor.models.reduce((modelSum, model) => {
      ...
      return new Metrics({
        ...
        total_suggestions_count: breakdown.reduce(
          (sum, item) => sum + (item.suggestions_count || 0),
          0,
        ),
        total_acceptances_count: breakdown.reduce(
          (sum, item) => sum + (item.acceptances_count || 0),
          0,
        ),
        total_lines_suggested: breakdown.reduce(
          (sum, item) => sum + (item.lines_suggested || 0),
          0,
        ),
        total_lines_accepted: breakdown.reduce(
          (sum, item) => sum + (item.lines_accepted || 0),
          0,
        ),
