...
test.describe("Teams Comparison tests", () => {
  ...
  test("teams comparison functionality with team selection", tag, async () => {
    ...
    const theATeamOption = dashboard.page
      .locator('[role="listbox"]')
      .getByText("The A Team")
      .first();
    ...
    const devTeamOption = dashboard.page
      .locator('[role="listbox"]')
    ...
    const totalActiveUsersCard = dashboard.page.locator("div.text-h6.mb-1", {
      hasText: "Total Active Users",
    });
    await expect(totalActiveUsersCard).toBeVisible();

    // Verify that charts are displayed
    const languageUsageChart = dashboard.page.getByText(
      "Language Usage by Team",
    );
  ...
  test("teams comparison empty state", tag, async () => {
    ...
    const emptyStateDescription = dashboard.page.getByText(
