...
export class GitHubTab {
  ...
  constructor(page: Page) {
    ...
    this.statisticsTitle = page
      .locator("h2")
      .filter({ hasText: "Copilot Statistics" });
    this.dateRangeCard = page
      .locator(".v-card")
      .filter({ hasText: /calendar-range/ });

    // Overview cards - using more specific selectors to avoid ambiguity
    this.ideCodeCompletionsCard = page
      .locator(".v-card")
      .filter({ hasText: "IDE Code Completions" })
      .first();
    this.ideChatCard = page
      .locator(".v-card")
      .filter({ hasText: "IDE Chat" })
      .first();
    this.githubChatCard = page
      .locator(".v-card")
      .filter({ hasText: "GitHub.com Chat" })
      .first();
    this.githubPRSummariesCard = page
      .locator(".v-card")
      .filter({ hasText: "GitHub.com PR Summaries" })
      .first();

    // Chart sections
    this.featureUsageTitle = page
      .locator("h2")
      .filter({ hasText: "Copilot Feature Usage Over Time" });
    this.modelUsageTitle = page
      .locator("h2")
      .filter({ hasText: "Model Usage Distribution" });
    this.chartContainers = page.locator(".chart-container");

    // Models section - using correct CSS selectors
    this.modelsTitle = page
      .locator("h2")
      .filter({ hasText: "Models Used by Users" });
    ...
    // Legacy locators
    this.cumulativeNumberOfTurnsLabel = page.getByText(
      "Cumulative Number of Turns",
    );
    this.cumulativeNumberOfTurnsValue = page
      .locator(".v-card-item")
      .filter({ has: this.cumulativeNumberOfTurnsLabel })
      .locator(".text-h4");
  }
  ...
  async monitorPerformanceWarnings() {
    ...
    this.page.once("console", (msg) => {
      if (
        msg.type() === "warning" &&
        msg.text().includes("requestAnimationFrame")
      ) {
